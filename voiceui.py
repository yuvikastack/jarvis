...
# Streamlit UI Setup
st.set_page_config(page_title="My Voice Bot", page_icon="🎙️", layout="centered")
st.title("My Voice Bot")
st.write("Click the 'Start' button to activate your Jarvis voice bot.")
...
if st.button("Start"):
    st.success("Voice bot is starting...")
    
    # Run the voice bot loop inside the UI
    while True:
        st.write("Speak your command...")
        user_input = listen()
        st.write(f"You said: {user_input}")
        response = Automation(user_input)
        st.write(f"{Assistantname}: {response}")
        speak(response)
